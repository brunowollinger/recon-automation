...
def executa(sistema):
    subprocess.check_output(f'docker run --entrypoint bash --rm --name {container_name} -v /docker/data/{target}/tmp:/data kalilinux/kali-tools:2.0 -c "nuclei -u {sistema} -t /root/.local/nuclei-templates/ -je /data/{saida}" || true', shell=True)
